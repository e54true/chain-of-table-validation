...
def generate_prompt_for_next_step(
    ...
    if feedback:
        prompt = f"\nWarning: Your last choice operation: '{op_name}' was 'incorrect'. Please choose a new operation, and here is reason: {feedback}" + prompt
...
def dynamic_chain_exec_one_sample(
    ...
    while True:
        ...
        if validation_llm:
            ...
            The selected operation is: {next_operation}
            Do you think this operation is appropriate? Answer only 'yes' or 'no' first, and explain why.
            """

            validation_response = validation_llm.generate(
                validation_prompt,
                options = validation_llm_options
            )

            if "no" in validation_response.lower()[:2]:
                print("---rejected---")
                print(next_operation)
...
def _dynamic_chain_exec_with_cache_mp_core(arg):
    idx, sample, llm, validation_llm, llm_options, validation_llm_options, strategy, cache_dir = arg
    ...
    try:
        ...
        if os.path.exists(cache_path):
            ...
            proc_sample, log = dynamic_chain_exec_one_sample(
                sample, llm=llm, validation_llm=validation_llm ,llm_options=llm_options, validation_llm_options=validation_llm_options, strategy=strategy
            )
            pickle.dump((sample, proc_sample, log), open(cache_path, "wb"))
...
def dynamic_chain_exec_with_cache_mp(
    ...
    args = [
        (idx, sample, llm, validation_llm, llm_options, validation_llm_options, strategy, cache_dir)
        for idx, sample in enumerate(all_samples)
    ]
